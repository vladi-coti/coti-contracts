...
function randomSigned8() {
  let unsigned = generateRandomNumber(1);
...
async function deploy() {
  ...
  const factory = await hre.ethers.getContractFactory("SignedInt8TestsContract")
...
describe("MPC Core - signed 8-bit integers", function () {
  ...
  describe("Validating encrypted signed 8-bit integers", function () {
    it("Should validate positive signed integers", async function () {
      ...
      expect(decryptedInt).to.equal(42)
    })
  ...
  describe("Subtracting signed 8-bit integers", function () {
    it("Should encrypt, subtract and decrypt two positive signed integers", async function () {
      const { contract } = deployment
      await (await contract.subTest(20, 10)).wait()
  ...
  describe("Multiplying signed 8-bit integers", function () {
  ...
  describe("Dividing signed 8-bit integers", function () {
    it("Should encrypt, divide and decrypt two positive signed integers", async function () {
      const { contract } = deployment
      await (await contract.divTest(20, 10)).wait()
  ...
  describe("AND signed 8-bit integers", function () {
  ...
  describe("OR signed 8-bit integers", function () {
  ...
  describe("XOR signed 8-bit integers", function () {
  ...
  describe("EQ signed 8-bit integers", function () {
    it("should return true for eqTest(5, 5) (equal pos)", async function () {
      const { contract } = deployment;
      await (await contract.eqTest(5n, 5n)).wait();
      expect(await contract.eqResult()).to.equal(true);
    });
    it("should return true for eqTest(-5, -5) (equal neg)", async function () {
      const { contract } = deployment;
      await (await contract.eqTest(-5n, -5n)).wait();
      expect(await contract.eqResult()).to.equal(true);
    });
    it("should return false for eqTest(5, -5) (pos, neg)", async function () {
      const { contract } = deployment;
      await (await contract.eqTest(5n, -5n)).wait();
      expect(await contract.eqResult()).to.equal(false);
    });
    it("should return false for eqTest(-5, 5) (neg, pos)", async function () {
      const { contract } = deployment;
      await (await contract.eqTest(-5n, 5n)).wait();
      expect(await contract.eqResult()).to.equal(false);
    });
    it("should return false for eqTest(5, 3) (pos, pos)", async function () {
      const { contract } = deployment;
      await (await contract.eqTest(5n, 3n)).wait();
      expect(await contract.eqResult()).to.equal(false);
    });
    it("should return false for eqTest(-5, -3) (neg, neg)", async function () {
      const { contract } = deployment;
      await (await contract.eqTest(-5n, -3n)).wait();
      expect(await contract.eqResult()).to.equal(false);
    });
  ...
  describe("NE signed 8-bit integers", function () {
    it("should return false for neTest(5, 5) (equal pos)", async function () {
      const { contract } = deployment;
      await (await contract.neTest(5n, 5n)).wait();
      expect(await contract.neResult()).to.equal(false);
    });
    it("should return false for neTest(-5, -5) (equal neg)", async function () {
      const { contract } = deployment;
      await (await contract.neTest(-5n, -5n)).wait();
      expect(await contract.neResult()).to.equal(false);
    });
    it("should return true for neTest(5, -5) (pos, neg)", async function () {
      const { contract } = deployment;
      await (await contract.neTest(5n, -5n)).wait();
      expect(await contract.neResult()).to.equal(true);
    });
    it("should return true for neTest(-5, 5) (neg, pos)", async function () {
      const { contract } = deployment;
      await (await contract.neTest(-5n, 5n)).wait();
      expect(await contract.neResult()).to.equal(true);
    });
    it("should return true for neTest(5, 3) (pos, pos)", async function () {
      const { contract } = deployment;
      await (await contract.neTest(5n, 3n)).wait();
      expect(await contract.neResult()).to.equal(true);
    });
    it("should return true for neTest(-5, -3) (neg, neg)", async function () {
      const { contract } = deployment;
      await (await contract.neTest(-5n, -3n)).wait();
      expect(await contract.neResult()).to.equal(true);
    });
  ...
  describe("Offboard signed 8-bit integers", function () {
    ...
    it("Should decrypt the positive signed integers", async function () {
      ...
      const decryptedInt = await owner.decryptInt8(encryptedInt)
      expect(decryptedInt).to.equal(3)
    })
  ...
  describe("GT signed 8-bit integers", function () {
    it("should return true for gtTest(5, 3) (pos, pos)", async function () {
      const { contract } = deployment;
      await (await contract.gtTest(5n, 3n)).wait();
      expect(await contract.gtResult()).to.equal(true);
    });
    it("should return true for gtTest(5, -3) (pos, neg)", async function () {
      const { contract } = deployment;
      await (await contract.gtTest(5n, -3n)).wait();
      expect(await contract.gtResult()).to.equal(true);
    });
    it("should return false for gtTest(-5, 3) (neg, pos)", async function () {
      const { contract } = deployment;
      await (await contract.gtTest(-5n, 3n)).wait();
      expect(await contract.gtResult()).to.equal(false);
    });
    it("should return false for gtTest(-5, -3) (neg, neg)", async function () {
      const { contract } = deployment;
      await (await contract.gtTest(-5n, -3n)).wait();
      expect(await contract.gtResult()).to.equal(false);
    });
    it("should return false for gtTest(5, 5) (equal pos)", async function () {
      const { contract } = deployment;
      await (await contract.gtTest(5n, 5n)).wait();
      expect(await contract.gtResult()).to.equal(false);
    });
    it("should return false for gtTest(-5, -5) (equal neg)", async function () {
      const { contract } = deployment;
      await (await contract.gtTest(-5n, -5n)).wait();
      expect(await contract.gtResult()).to.equal(false);
    });
  ...
  describe("GE signed 8-bit integers", function () {
    it("should return true for geTest(5, 3) (pos, pos)", async function () {
      const { contract } = deployment;
      await (await contract.geTest(5n, 3n)).wait();
      expect(await contract.geResult()).to.equal(true);
    });
    it("should return true for geTest(5, -3) (pos, neg)", async function () {
      const { contract } = deployment;
      await (await contract.geTest(5n, -3n)).wait();
      expect(await contract.geResult()).to.equal(true);
    });
    it("should return false for geTest(-5, 3) (neg, pos)", async function () {
      const { contract } = deployment;
      await (await contract.geTest(-5n, 3n)).wait();
      expect(await contract.geResult()).to.equal(false);
    });
    it("should return false for geTest(-5, -3) (neg, neg)", async function () {
      const { contract } = deployment;
      await (await contract.geTest(-5n, -3n)).wait();
      expect(await contract.geResult()).to.equal(false);
    });
    it("should return true for geTest(5, 5) (equal pos)", async function () {
      const { contract } = deployment;
      await (await contract.geTest(5n, 5n)).wait();
      expect(await contract.geResult()).to.equal(true);
    });
    it("should return true for geTest(-5, -5) (equal neg)", async function () {
      const { contract } = deployment;
      await (await contract.geTest(-5n, -5n)).wait();
      expect(await contract.geResult()).to.equal(true);
    });
  ...
  describe("LT signed 8-bit integers", function () {
    it("should return false for ltTest(5, 3) (pos, pos)", async function () {
      const { contract } = deployment;
      await (await contract.ltTest(5n, 3n)).wait();
      expect(await contract.ltResult()).to.equal(false);
    });
    it("should return false for ltTest(5, -3) (pos, neg)", async function () {
      const { contract } = deployment;
      await (await contract.ltTest(5n, -3n)).wait();
      expect(await contract.ltResult()).to.equal(false);
    });
    it("should return true for ltTest(-5, 3) (neg, pos)", async function () {
      const { contract } = deployment;
      await (await contract.ltTest(-5n, 3n)).wait();
      expect(await contract.ltResult()).to.equal(true);
    });
    it("should return true for ltTest(-5, -3) (neg, neg)", async function () {
      const { contract } = deployment;
      await (await contract.ltTest(-5n, -3n)).wait();
      expect(await contract.ltResult()).to.equal(true);
    });
    it("should return false for ltTest(5, 5) (equal pos)", async function () {
      const { contract } = deployment;
      await (await contract.ltTest(5n, 5n)).wait();
      expect(await contract.ltResult()).to.equal(false);
    });
    it("should return false for ltTest(-5, -5) (equal neg)", async function () {
      const { contract } = deployment;
      await (await contract.ltTest(-5n, -5n)).wait();
      expect(await contract.ltResult()).to.equal(false);
    });
  ...
  describe("LE signed 8-bit integers", function () {
    it("should return false for leTest(5, 3) (pos, pos)", async function () {
      const { contract } = deployment;
      await (await contract.leTest(5n, 3n)).wait();
      expect(await contract.leResult()).to.equal(false);
    });
    it("should return false for leTest(5, -3) (pos, neg)", async function () {
      const { contract } = deployment;
      await (await contract.leTest(5n, -3n)).wait();
      expect(await contract.leResult()).to.equal(false);
    });
    it("should return true for leTest(-5, 3) (neg, pos)", async function () {
      const { contract } = deployment;
      await (await contract.leTest(-5n, 3n)).wait();
      expect(await contract.leResult()).to.equal(true);
    });
    it("should return true for leTest(-5, -3) (neg, neg)", async function () {
      const { contract } = deployment;
      await (await contract.leTest(-5n, -3n)).wait();
      expect(await contract.leResult()).to.equal(true);
    });
    it("should return true for leTest(5, 5) (equal pos)", async function () {
      const { contract } = deployment;
      await (await contract.leTest(5n, 5n)).wait();
      expect(await contract.leResult()).to.equal(true);
    });
    it("should return true for leTest(-5, -5) (equal neg)", async function () {
      const { contract } = deployment;
      await (await contract.leTest(-5n, -5n)).wait();
      expect(await contract.leResult()).to.equal(true);
    });
  ...
  describe("Edge cases for signed 8-bit", function () {
    const testCases = [
      { a: 0n, b: 0n },
      { a: 1n, b: 0n },
      { a: 0n, b: 1n },
    ...
    for (const { a, b } of testCases) {
      it(`edge case 8-bit addTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        await (await contract.addTest(a, b, gasOptions)).wait()
        const decryptedInt = await contract.addResult()
        expect(decryptedInt).to.equal(BigInt.asIntN(8, a + b))
      })
      it(`edge case 8-bit subTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        await (await contract.subTest(a, b, gasOptions)).wait()
        const decryptedInt = await contract.subResult()
        expect(decryptedInt).to.equal(BigInt.asIntN(8, a - b))
      })
      it(`edge case 8-bit mulTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        await (await contract.mulTest(a, b, gasOptions)).wait()
        const decryptedInt = await contract.mulResult()
        expect(decryptedInt).to.equal(BigInt.asIntN(8, a * b))
      })
      it(`edge case 8-bit divTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        if(b === 0n) {
          let revert = false
          try {
      ...
      it(`edge case 8-bit ltTest(${a}, ${b})`, async function () {
        const { contract } = deployment
  ...
  describe("Fuzz testing signed 8-bit arithmetic", function () {
    let deployment: Awaited<ReturnType<typeof deploy>>
    before(async function () { deployment = await deploy() })
    for (let i = 0; i < 10; i++) {
      const a = randomSigned8()
      const b = randomSigned8()
      it(`fuzz 8-bit addTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        await (await contract.addTest(a, b, gasOptions)).wait()
        const decryptedInt = await contract.addResult()
        expect(decryptedInt).to.equal(BigInt.asIntN(8, a + b))
      })
      it(`fuzz 8-bit subTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        await (await contract.subTest(a, b, gasOptions)).wait()
        const decryptedInt = await contract.subResult()
        expect(decryptedInt).to.equal(BigInt.asIntN(8, a - b))
      })
      it(`fuzz 8-bit mulTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        await (await contract.mulTest(a, b, gasOptions)).wait()
        const decryptedInt = await contract.mulResult()
        expect(decryptedInt).to.equal(BigInt.asIntN(8, a * b))
      })
      it(`fuzz 8-bit divTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        if(b === 0n) {
          let revert = false
          try {
      ...
      it(`fuzz 8-bit andTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        await (await contract.andTest(a, b, gasOptions)).wait()
        const decryptedInt = await contract.andResult()
        expect(decryptedInt).to.equal(BigInt.asIntN(8, a & b))
      })
      it(`fuzz 8-bit orTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        await (await contract.orTest(a, b, gasOptions)).wait()
        const decryptedInt = await contract.orResult()
        expect(decryptedInt).to.equal(BigInt.asIntN(8, a | b))
      })
      it(`fuzz 8-bit xorTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        await (await contract.xorTest(a, b, gasOptions)).wait()
        const decryptedInt = await contract.xorResult()
        expect(decryptedInt).to.equal(BigInt.asIntN(8, a ^ b))
      })
      it(`fuzz 8-bit eqTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        await (await contract.eqTest(a, b, gasOptions)).wait()
      ...
      it(`fuzz 8-bit neTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        await (await contract.neTest(a, b, gasOptions)).wait()
      ...
      it(`fuzz 8-bit ltTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        await (await contract.ltTest(a, b, gasOptions)).wait()
      ...
      it(`fuzz 8-bit leTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        await (await contract.leTest(a, b, gasOptions)).wait()
      ...
      it(`fuzz 8-bit gtTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        await (await contract.gtTest(a, b, gasOptions)).wait()
      ...
      it(`fuzz 8-bit geTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        await (await contract.geTest(a, b, gasOptions)).wait()
