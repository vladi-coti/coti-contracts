...
function randomSigned16() {
  let unsigned = generateRandomNumber(2);
  // If the most significant bit is set (i.e. sign bit), convert to signed using two's complement
  const signBit = 1n << 15n;
  if (unsigned & signBit) {
      unsigned = unsigned - (1n << 16n);
  }
  return unsigned;
}
...
async function deploy() {
  ...
  const factory = await hre.ethers.getContractFactory("SignedInt16TestsContract")
...
describe("MPC Core - signed 16-bit integers", function () {
  ...
  describe("Offboard signed integers", function () {
    ...
    it("Should decrypt the negative signed integers", async function () {
      ...
      const decryptedInt = await owner.decryptInt16(encryptedInt)

      expect(decryptedInt).to.equal(-11n)
    })
  })

  describe("GT signed 16-bit integers", function () {
    it("should return true for gtTest(123, 45) (pos, pos)", async function () {
      const { contract } = deployment;
      await (await contract.gtTest(123n, 45n)).wait();
      expect(await contract.gtResult()).to.equal(true);
    });
    it("should return true for gtTest(123, -45) (pos, neg)", async function () {
      const { contract } = deployment;
      await (await contract.gtTest(123n, -45n)).wait();
      expect(await contract.gtResult()).to.equal(true);
    });
    it("should return false for gtTest(-123, 45) (neg, pos)", async function () {
      const { contract } = deployment;
      await (await contract.gtTest(-123n, 45n)).wait();
      expect(await contract.gtResult()).to.equal(false);
    });
    it("should return false for gtTest(-123, -45) (neg, neg)", async function () {
      const { contract } = deployment;
      await (await contract.gtTest(-123n, -45n)).wait();
      expect(await contract.gtResult()).to.equal(false);
    });
    it("should return false for gtTest(123, 123) (equal pos)", async function () {
      const { contract } = deployment;
      await (await contract.gtTest(123n, 123n)).wait();
      expect(await contract.gtResult()).to.equal(false);
    });
    it("should return false for gtTest(-123, -123) (equal neg)", async function () {
      const { contract } = deployment;
      await (await contract.gtTest(-123n, -123n)).wait();
      expect(await contract.gtResult()).to.equal(false);
    });
  ...
  describe("GE signed 16-bit integers", function () {
    it("should return true for geTest(123, 45) (pos, pos)", async function () {
      const { contract } = deployment;
      await (await contract.geTest(123n, 45n)).wait();
      expect(await contract.geResult()).to.equal(true);
    });
    it("should return true for geTest(123, -45) (pos, neg)", async function () {
      const { contract } = deployment;
      await (await contract.geTest(123n, -45n)).wait();
      expect(await contract.geResult()).to.equal(true);
    });
    it("should return false for geTest(-123, 45) (neg, pos)", async function () {
      const { contract } = deployment;
      await (await contract.geTest(-123n, 45n)).wait();
      expect(await contract.geResult()).to.equal(false);
    });
    it("should return false for geTest(-123, -45) (neg, neg)", async function () {
      const { contract } = deployment;
      await (await contract.geTest(-123n, -45n)).wait();
      expect(await contract.geResult()).to.equal(false);
    });
    it("should return true for geTest(123, 123) (equal pos)", async function () {
      const { contract } = deployment;
      await (await contract.geTest(123n, 123n)).wait();
      expect(await contract.geResult()).to.equal(true);
    });
    it("should return true for geTest(-123, -123) (equal neg)", async function () {
      const { contract } = deployment;
      await (await contract.geTest(-123n, -123n)).wait();
      expect(await contract.geResult()).to.equal(true);
    });
  ...
  describe("LT signed 16-bit integers", function () {
    it("should return false for ltTest(123, 45) (pos, pos)", async function () {
      const { contract } = deployment;
      await (await contract.ltTest(123n, 45n)).wait();
      expect(await contract.ltResult()).to.equal(false);
    });
    it("should return false for ltTest(123, -45) (pos, neg)", async function () {
      const { contract } = deployment;
      await (await contract.ltTest(123n, -45n)).wait();
      expect(await contract.ltResult()).to.equal(false);
    });
    it("should return true for ltTest(-123, 45) (neg, pos)", async function () {
      const { contract } = deployment;
      await (await contract.ltTest(-123n, 45n)).wait();
      expect(await contract.ltResult()).to.equal(true);
    });
    it("should return true for ltTest(-123, -45) (neg, neg)", async function () {
      const { contract } = deployment;
      await (await contract.ltTest(-123n, -45n)).wait();
      expect(await contract.ltResult()).to.equal(true);
    });
    it("should return false for ltTest(123, 123) (equal pos)", async function () {
      const { contract } = deployment;
      await (await contract.ltTest(123n, 123n)).wait();
      expect(await contract.ltResult()).to.equal(false);
    });
    it("should return false for ltTest(-123, -123) (equal neg)", async function () {
      const { contract } = deployment;
      await (await contract.ltTest(-123n, -123n)).wait();
      expect(await contract.ltResult()).to.equal(false);
    });
  ...
  describe("LE signed 16-bit integers", function () {
    it("should return false for leTest(123, 45) (pos, pos)", async function () {
      const { contract } = deployment;
      await (await contract.leTest(123n, 45n)).wait();
      expect(await contract.leResult()).to.equal(false);
    });
    it("should return false for leTest(123, -45) (pos, neg)", async function () {
      const { contract } = deployment;
      await (await contract.leTest(123n, -45n)).wait();
      expect(await contract.leResult()).to.equal(false);
    });
    it("should return true for leTest(-123, 45) (neg, pos)", async function () {
      const { contract } = deployment;
      await (await contract.leTest(-123n, 45n)).wait();
      expect(await contract.leResult()).to.equal(true);
    });
    it("should return true for leTest(-123, -45) (neg, neg)", async function () {
      const { contract } = deployment;
      await (await contract.leTest(-123n, -45n)).wait();
      expect(await contract.leResult()).to.equal(true);
    });
    it("should return true for leTest(123, 123) (equal pos)", async function () {
      const { contract } = deployment;
      await (await contract.leTest(123n, 123n)).wait();
      expect(await contract.leResult()).to.equal(true);
    });
    it("should return true for leTest(-123, -123) (equal neg)", async function () {
      const { contract } = deployment;
      await (await contract.leTest(-123n, -123n)).wait();
      expect(await contract.leResult()).to.equal(true);
    });
  ...
  describe("EQ signed 16-bit integers", function () {
    it("should return true for eqTest(123, 123) (equal pos)", async function () {
      const { contract } = deployment;
      await (await contract.eqTest(123n, 123n)).wait();
      expect(await contract.eqResult()).to.equal(true);
    });
    it("should return true for eqTest(-123, -123) (equal neg)", async function () {
      const { contract } = deployment;
      await (await contract.eqTest(-123n, -123n)).wait();
      expect(await contract.eqResult()).to.equal(true);
    });
    it("should return false for eqTest(123, -123) (pos, neg)", async function () {
      const { contract } = deployment;
      await (await contract.eqTest(123n, -123n)).wait();
      expect(await contract.eqResult()).to.equal(false);
    });
    it("should return false for eqTest(-123, 123) (neg, pos)", async function () {
      const { contract } = deployment;
      await (await contract.eqTest(-123n, 123n)).wait();
      expect(await contract.eqResult()).to.equal(false);
    });
    it("should return false for eqTest(123, 45) (pos, pos)", async function () {
      const { contract } = deployment;
      await (await contract.eqTest(123n, 45n)).wait();
      expect(await contract.eqResult()).to.equal(false);
    });
    it("should return false for eqTest(-123, -45) (neg, neg)", async function () {
      const { contract } = deployment;
      await (await contract.eqTest(-123n, -45n)).wait();
      expect(await contract.eqResult()).to.equal(false);
    });
  ...
  describe("NE signed 16-bit integers", function () {
    it("should return false for neTest(123, 123) (equal pos)", async function () {
      const { contract } = deployment;
      await (await contract.neTest(123n, 123n)).wait();
      expect(await contract.neResult()).to.equal(false);
    });
    it("should return false for neTest(-123, -123) (equal neg)", async function () {
      const { contract } = deployment;
      await (await contract.neTest(-123n, -123n)).wait();
      expect(await contract.neResult()).to.equal(false);
    });
    it("should return true for neTest(123, -123) (pos, neg)", async function () {
      const { contract } = deployment;
      await (await contract.neTest(123n, -123n)).wait();
      expect(await contract.neResult()).to.equal(true);
    });
    it("should return true for neTest(-123, 123) (neg, pos)", async function () {
      const { contract } = deployment;
      await (await contract.neTest(-123n, 123n)).wait();
      expect(await contract.neResult()).to.equal(true);
    });
    it("should return true for neTest(123, 45) (pos, pos)", async function () {
      const { contract } = deployment;
      await (await contract.neTest(123n, 45n)).wait();
      expect(await contract.neResult()).to.equal(true);
    });
    it("should return true for neTest(-123, -45) (neg, neg)", async function () {
      const { contract } = deployment;
      await (await contract.neTest(-123n, -45n)).wait();
      expect(await contract.neResult()).to.equal(true);
    });
  ...
  describe("Edge cases for signed 16-bit", function () {
    const MAX = (1n << 15n) - 1n;
    const MIN = -(1n << 15n);
    const testCases = [
      { a: 0n, b: 0n },
    ...
    for (const { a, b } of testCases) {
      it(`edge case 16-bit addTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
        await (await contract.addTest(a, b, gasOptions)).wait();
        const decryptedInt = await contract.addResult();
        expect(decryptedInt).to.equal(BigInt.asIntN(16, a + b));
      });
      it(`edge case 16-bit subTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
        await (await contract.subTest(a, b, gasOptions)).wait();
        const decryptedInt = await contract.subResult();
        expect(decryptedInt).to.equal(BigInt.asIntN(16, a - b));
      });
      it(`edge case 16-bit mulTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
        await (await contract.mulTest(a, b, gasOptions)).wait();
        const decryptedInt = await contract.mulResult();
        expect(decryptedInt).to.equal(BigInt.asIntN(16, a * b));
      });
      it(`edge case 16-bit divTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
        if(b === 0n) {
          let revert = false
          try {
      ...
      it(`edge case 16-bit ltTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
  ...
  describe("Fuzz testing signed 16-bit arithmetic", function () {
    let deployment: Awaited<ReturnType<typeof deploy>>;
    before(async function () { deployment = await deploy(); });
    for (let i = 0; i < 10; i++) {
      const a = randomSigned16();
      const b = randomSigned16();
      it(`fuzz 16-bit addTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
        await (await contract.addTest(a, b, gasOptions)).wait();
        const decryptedInt = await contract.addResult();
        expect(decryptedInt).to.equal(BigInt.asIntN(16, a + b));
      });
      it(`fuzz 16-bit subTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
        await (await contract.subTest(a, b, gasOptions)).wait();
        const decryptedInt = await contract.subResult();
        expect(decryptedInt).to.equal(BigInt.asIntN(16, a - b));
      });
      it(`fuzz 16-bit mulTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
        await (await contract.mulTest(a, b, gasOptions)).wait();
        const decryptedInt = await contract.mulResult();
        expect(decryptedInt).to.equal(BigInt.asIntN(16, a * b));
      });
      it(`fuzz 16-bit divTest(${a}, ${b})`, async function () {
        const { contract } = deployment
        if(b === 0n) {
          let revert = false
          try {
      ...
      it(`fuzz 16-bit andTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
        await (await contract.andTest(a, b, gasOptions)).wait();
        const decryptedInt = await contract.andResult();
        expect(decryptedInt).to.equal(BigInt.asIntN(16, a & b));
      });
      it(`fuzz 16-bit orTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
        await (await contract.orTest(a, b, gasOptions)).wait();
        const decryptedInt = await contract.orResult();
        expect(decryptedInt).to.equal(BigInt.asIntN(16, a | b));
      });
      it(`fuzz 16-bit xorTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
        await (await contract.xorTest(a, b, gasOptions)).wait();
        const decryptedInt = await contract.xorResult();
        expect(decryptedInt).to.equal(BigInt.asIntN(16, a ^ b));
      });
      it(`fuzz 16-bit eqTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
        await (await contract.eqTest(a, b, gasOptions)).wait();
      ...
      it(`fuzz 16-bit neTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
        await (await contract.neTest(a, b, gasOptions)).wait();
      ...
      it(`fuzz 16-bit ltTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
        await (await contract.ltTest(a, b, gasOptions)).wait();
      ...
      it(`fuzz 16-bit leTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
        await (await contract.leTest(a, b, gasOptions)).wait();
      ...
      it(`fuzz 16-bit gtTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
        await (await contract.gtTest(a, b, gasOptions)).wait();
      ...
      it(`fuzz 16-bit geTest(${a}, ${b})`, async function () {
        const { contract } = deployment;
        await (await contract.geTest(a, b, gasOptions)).wait();
